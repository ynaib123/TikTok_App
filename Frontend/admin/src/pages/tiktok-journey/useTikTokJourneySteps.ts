...
import { isRenderReady, mergeIdeasById } from './journeyHelpers'
...
export interface UseCreationStepOptions {
  ...
  setUploadResult: SetState<UploadTikTokMediaResponse | null>
  ...
  const handleGenerateIdea = async () =>
    runAction('generateIdea', async () => {
      ...
      const workflowResponse = await triggerMainContentPipeline({
        ...
        sceneCount:
          typeof generationSceneCount === 'number' && generationSceneCount > 0
            ? generationSceneCount
            : null,
        force: true,
      })
      ...
      try {
        ...
        showSuccess(
          'Idee et script generes. Tu peux la regenerer ou valider pour passer a la video.',
        )
      } catch (error) {
        markWorkflowFinished({
          runId: workflowResponse?.runId || null,
...
export interface UseRenderStepOptions {
  ...
  waitForContentIdeaStatus: (
    ideaId: number | string,
    predicate: (s: ContentIdeaStatus | null) => boolean,
    timeoutMs?: number,
  ) => Promise<ContentIdeaStatus | null>
  waitForRenderedVideo: (
    ideaId: number | string,
    checkRenderStatus?: (() => Promise<unknown>) | null,
  ) => Promise<ContentIdea>
  refreshPipelineData: () => Promise<unknown>
  setScriptedIdea: SetState<ContentIdea | null>
  setGeneratedIdeas: SetState<ContentIdea[]>
  ...
  const handleValidateScript = async () =>
    runAction('renderVideo', async () => {
      ...
      const workflowRun = await triggerRenderTemplateWorkflow({
        ...
        durationSec:
          typeof videoDurationSec === 'number' && videoDurationSec > 0 ? videoDurationSec : null,
        sceneCount:
          typeof generationSceneCount === 'number' && generationSceneCount > 0
            ? generationSceneCount
            : null,
        selectedSceneMediaUrls:
          Array.isArray(selectedSceneMediaUrls) && selectedSceneMediaUrls.length > 0
            ? selectedSceneMediaUrls
            : null,
        sceneTextStyles:
          Array.isArray(sceneTextStyles) && sceneTextStyles.length > 0 ? sceneTextStyles : null,
      })
      ...
      if (completedRun && String(completedRun.status || '').toUpperCase() === 'SUCCEEDED') {
        const status = await waitForContentIdeaStatus(
          Number(idea.id),
          (candidate) => candidate?.pipelineStage === 'RENDER_READY',
          20_000,
        )
...
export interface UseUploadStepOptions {
  ...
  uploadTikTokMedia: (input: {
    id: number | string
    shotstackUrl: string
    uploadUrl: string
    force?: boolean
  }) => Promise<UploadTikTokMediaResponse>
  fetchContentIdeaById: (ideaId: number | string) => Promise<ContentIdea | null>
...
export function useUploadStep({
  ...
  const uploadVideo = async (actionOverride?: ManualActionState | null) => {
    ...
    if (!idea?.id || !action?.shotstackUrl || !action?.uploadUrl) {
      throw new Error(
        'Il manque la video ou les informations necessaires pour lancer la publication.',
      )
    }
    ...
    try {
      ...
      if (refreshedIdea && isUploadCompleted(refreshedIdea)) {
        ...
        showSuccess(
          'Publication terminee cote serveur. La reponse HTTP a probablement expire, mais la video est bien passee en statut upload.',
        )
        return
      }
  ...
  const prepareUpload = async () => {
    ...
    const workflowRun = await triggerPublishTikTokWorkflow({
      ...
      contentIdeaId: idea.id,
      topic: idea.topic,
      tiktokSoundId: selectedTikTokSoundId ?? null,
    })
    markWorkflowStarted({
      runId: workflowRun?.runId,
    ...
    const nextManualAction = await raceWorkflowRunAndUploadPreparation(
      workflowRun?.runId,
      Number(idea.id),
    )
