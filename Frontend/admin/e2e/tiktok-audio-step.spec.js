...
test('AudioStep renders voices, plays a preview, generates a take', async ({ page }) => {
  await page.route('**/api/admins/csrf-token', (route) =>
    route.fulfill({ json: { token: 't' } }))

  await page.route('**/api/admins/me', (route) =>
    route.fulfill({ json: {
      id: 1, email: 'admin@local', nom: 'Admin', role: 'ADMIN',
    } }))
  ...
  await page.route('**/api/audio/assets/**', (route) =>
    route.fulfill({ json: [] }))
  ...
  await expect(page.getByRole('listitem').filter({ hasText: 'Rachel' })).toBeVisible({ timeout: 15_000 })
  await expect(page.getByRole('listitem').filter({ hasText: 'Antoine' })).toBeVisible()

  await page.getByRole('button', { name: /Sélectionner cette voix|Selected/i }).first().click()
  await page.getByRole('button', { name: /Générer la voix off/i }).click()

  await expect.poll(() => generated, { timeout: 10_000 }).toBeGreaterThan(0)
})
